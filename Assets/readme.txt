...
1) Можно остановить матч с задержкой в секунду. Для того, чтобы игрокам пришло уведомлениие об окончании
    матча. Это костыль, но 99,9% игроков получат сообщение. Если сеть упадёт в момент отправки на время 
    больше секунды, то игроки зависнут.
2) Можно вынести rudp в отдельное хранилище, в котором будут храниться rudp всех игроков. Это обязательно нужно будет 
сделать после того, как сервер станет устойчивым. 

//TODO Нужна общая структура данных для rudp. Иначе удалять старые rudp сообщения можно только полным перебором матчей.
